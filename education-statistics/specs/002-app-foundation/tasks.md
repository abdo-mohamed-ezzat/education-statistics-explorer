...
- [ ] T004 [P] Create `UserPreferences` model, types (`Language`, `Direction`, `Theme`), and `DEFAULT_PREFERENCES` constant in `education-statistics/src/app/core/models/user-preferences.model.ts`
- [ ] T005 [P] Create `NavItem` interface in `education-statistics/src/app/core/models/nav-item.model.ts`
- [ ] T006 [P] Create `DatasetKey` and `DatasetLoadStatus` types in `education-statistics/src/app/core/models/dataset.model.ts`
- [ ] T007 [P] Create `ViewState<T>` discriminated union and factory helpers (`ViewStateLoading`, `ViewStateEmpty`, `viewStateData`, `viewStateError`) in `education-statistics/src/app/shared/models/view-state.model.ts`
- [ ] T008 Implement `PreferencesService` (signals: `language`, `direction` computed, `theme`; methods: `toggleLanguage`, `toggleTheme`, `setLanguage`, `setTheme`; `effect()` for `dir`/theme/`localStorage`/Transloco side-effects; `PLATFORM_ID` + `isPlatformBrowser` guard; initial lang detection from `localStorage` then browser `navigator.language`) in `education-statistics/src/app/core/services/preferences.service.ts` (depends on T004)
- [ ] T009 Implement `EducationDataService` (three `shareReplay(1)` cached `HttpClient` observables: `getRecords()`, `getMaster()`, `getSummary()`; strong return types; errors propagate — NOT swallowed) in `education-statistics/src/app/core/services/education-data.service.ts` (depends on T006)
...
- [ ] T010 [P] [US1] Implement `TopbarComponent` (dumb; inputs: `lang`, `theme`; outputs: `languageToggle`, `themeToggle`; displays app title; uses `ds-glassbar` + design system classes; Lucide icons for lang/theme toggles; accessible button targets with `aria-label` from translation keys) in `education-statistics/src/app/core/layout/topbar/topbar.component.ts` (depends on T004)
- [ ] T011 [P] [US1] Implement `NavComponent` (dumb; input: `items: NavItem[]`, `activeRoute: string`; output: `navigate: string`; renders nav links using Lucide icons + translation keys; `aria-current="page"` on active link; mobile-friendly; RTL-safe) in `education-statistics/src/app/core/layout/nav/nav.component.ts` (depends on T005)
- [ ] T012 [US1] Implement `ShellComponent` (smart container; injects `PreferencesService`, `Router`; defines `NAV_ITEMS: NavItem[]` constant for all 5 routes; reads `language()`, `theme()`, `direction()` signals; passes data to `TopbarComponent` and `NavComponent`; handles `languageToggle`/`themeToggle` outputs; contains `<router-outlet>`; uses `ds-shell` layout class; mobile-first responsive layout; RTL-safe) in `education-statistics/src/app/core/layout/shell/shell.component.ts` (depends on T008, T010, T011)
- [ ] T013 [US1] Simplify `AppComponent` to only render `<app-shell>` — strip all Angular default placeholder HTML and inline styles from `education-statistics/src/app/app.html` and `education-statistics/src/app/app.ts`
- [ ] T014 [US1] Add FOUC-prevention inline `<script>` to `<head>` of `education-statistics/src/index.html` — reads `localStorage('lang')` and `localStorage('theme')`, sets `document.documentElement.setAttribute('dir', ...)` and `document.documentElement.classList.add('dark'/'light')` synchronously before Angular boots
...
- [ ] T020 [US3] Verify `themes.css` has a `.dark` (or `[class~="dark"]`) selector block that pivots all `--color-*` tokens to dark-appropriate values — if the class-based dark pivot does not exist, add it to `education-statistics/src/styles/themes.css` (depends on T012)
- [ ] T021 [US3] Verify `PreferencesService` `effect()` uses `document.documentElement.classList.toggle('dark', theme === 'dark')` to apply/remove the dark class reactively on theme change, and that this is **also** done by the FOUC inline script in `index.html` — cross-check T008 and T014 outputs in `education-statistics/src/app/core/services/preferences.service.ts` + `education-statistics/src/index.html` (depends on T008, T014)
- [ ] T022 [US3] Verify theme toggle button in `TopbarComponent` uses a Lucide `sun` icon in dark mode and `moon` icon in light mode (or equivalent accessible toggle) and emits `themeToggle` output on click in `education-statistics/src/app/core/layout/topbar/topbar.component.ts` (depends on T010)
